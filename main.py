...
genai genai.configure(api_key=st.secrets["gemini_api"]) 
def ai(txt):
...
if command:
   ...
   if "hello" in command:
       ...
       with st.chat_message("bot"):
           st.write("Im ghost ai assistant")
           st.session_state.message.append({"role":"bot","message":" Im ghost ai assistant")) 
  elif "hi" in command:
      with st.chat_message("bot"):
